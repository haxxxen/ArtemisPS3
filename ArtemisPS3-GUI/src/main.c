            ...
            if (readPad(0))
            {
                ...
                else if(paddata[0].BTN_SQUARE)
                {
					if ((is_cobra() == SUCCESS))
					{
						...
						{lv2syscall4(11, SYSCALL11_OPCODE_GET_VSH_PLUGIN_INFO, 5, (uint64_t)plugin_name, (uint64_t)plugin_filename);}
						if (strlen(plugin_filename) >= 0 && strcmp(plugin_filename, (char *)"/dev_hdd0/game/ARTPS3001/USRDIR/artemis_ps3.sprx") == 0)
						{
							...
							{lv2syscall2(11, SYSCALL11_OPCODE_UNLOAD_VSH_PLUGIN, 5);}
							{lv2syscall3(392, 0x1004, 0x4, 0x6); } //1 Beep
						}
						else
						{
